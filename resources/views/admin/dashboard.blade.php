            ...
            <div class="tab-content" id="v-pills-tabContent">
                ...
                <div class="tab-pane fade" id="tab-propiedades" role="tabpanel">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <a href="{{ route('admin.create') }}" class="btn btn-success btn-sm fw-bold px-3 shadow-sm" style="border-radius: 20px;">
                        ...
                        <table class="table table-hover align-middle mb-0 text-center">
                            ...
                            <tbody>
                                @foreach($properties as $property)
                                    <tr>
                                        <td><img src="{{ $property->image_fachada }}" class="rounded" style="width: 55px; height: 35px; object-fit: cover;"></td>
                                        <td class="text-start fw-bold text-dark">{{ $property->title }}</td>
                                        <td><span class="badge bg-secondary-subtle text-secondary border px-2 py-1">{{ $property->type }}</span></td>
                                        <td class="fw-bold text-primary">${{ number_format($property->price, 2) }}</td>
                                        <td>
                                            <div class="d-flex gap-2 justify-content-center">
                                                <a href="{{ route('admin.edit', $property->id) }}" class="btn btn-sm btn-light border text-warning px-2"><i class="fa-solid fa-pen-to-square"></i></a>
                                                <form action="{{ route('admin.destroy', $property->id) }}" method="POST" onsubmit="return confirm('¿Confirmar baja permanente del catálogo?')">
                                                    @csrf @method('DELETE')
